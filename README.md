...
- 季數與集數都可以不填，沒填就不會出現在列表上
- 觀看狀態標籤：正在看 / 等更新 / 待看，可留空不標記
- 觀看時間一鍵歸零，新的一季直接重新計時
- 卡片式列表，片名完整顯示、可一鍵複製；編輯時儲存鍵固定在畫面底部
- 類別列表可依觀看狀態篩選，每次進入都是完整的全部紀錄
- 跨類別搜尋片名，支援語音輸入（瀏覽器支援時）
- 八種排序依據，設定會被記住

**快速檢視**
- 首頁一眼看到正在看 / 等更新 / 待看 / 未標記各有幾部
- 點進去依觀看類別分組顯示，陸劇、韓劇各歸各的，類別順序照你排的
- 四個狀態可在頁面上直接切換，不必回首頁

**外觀**
- 八種主題，深色淺色各四種：午夜紫 · 深海青 · 暗夜琥珀 · 純黑／晨光白 · 米紙 · 薄荷 · 櫻粉
- 點選即時套用，選擇記在本機、不會同步到雲端
- 手機主畫面啟動時狀態列會跟著主題變色
...
顏色一律走 CSS 變數（`src/assets/styles/tokens.css` 與 `themes.css`），元件內不出現字面色值——主題就是靠覆寫這層變數實作的。
